...
class ExtractTripParamsNode(BaseNode):
    ...
    async def async_run(self, state: TravelPlannerState) -> TravelPlannerState:  # type: ignore[override]
        ...
        if not isinstance(travel_params, TravelParams):
            self.logger.error(f"LLM response could not be parsed into TravelParams: {travel_params}")
        ...
        if missing_fields:
            self.logger.info(f"{self.node_id} | Missing or invalid travel parameters: {missing_fields}")
