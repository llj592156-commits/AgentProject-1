...
class TurkishAirlinesNode(BaseNode):
    ...
    def __init__(self, prompt_templates: PromptTemplates, llm_models: LLMs):
        ...
        self.logger.info(f"Turkish Airlines MCP Config - Node path: {self.node_bin_path}, MCP command: {self.mcp_remote_command}")
    ...
    async def async_run(self, state: TravelPlannerState) -> TravelPlannerState:  # type: ignore[override]
        try:
            ...
            self.logger.info(f"{self.node_id} | Processing Turkish Airlines query: {state.user_prompt}")
            
            agent_response = await self.turkish_airlines_agent.ainvoke({
            ...
            if agent_response and "messages" in agent_response and len(agent_response["messages"]) > 0:
                ...
                # Fallback response
                fallback_message = "I'm sorry, I couldn't process your Turkish Airlines request at the moment. Please try again later."
            ...
            error_message = "I encountered an issue while processing your Turkish Airlines request. Please try again or contact support if the problem persists."
